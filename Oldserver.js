  ...
  app.use(cors({ credentials: true, origin: 'https://novosisprati-990826f489ce.herokuapp.com/' }));
...
async function sendEmail(ID_Navio, ID_Agencia, dataRequisicao, isCondicionada, subject, ID_NavioSub) {
  ...
  const emailRecipients = await new Promise((resolve, reject) => {
    ...
    const selectedRebocador = dataRequisicao.selectedRebocador;
  ...
  let title = '';
  if (dataRequisicao.Servico === 'PUXADA' || service === 'TROCA DE BERÇO') {
  ...
  let emailContent = `
    ...
    <p><b> BERÇO: </b> ${isCondicionada ? dataRequisicao.Berco : dataRequisicao.berco_requi}</p>
    <p><b> POSIÇÃO: </b> ${isCondicionada ? dataRequisicao.Posicao_Berco : dataRequisicao.posicao_requi}</p>
...
app.post('/requisicoes', async (req, res) => {
  ...
  try {
    ...
    console.log('requisicaoDataFromCondicionado:',requisicaoDataFromCondicionado);

    // Create a new entry in the 'requisicoes' table for the original requisicao from frontend
    const insertQueryOriginal = `
      INSERT INTO requisicoes (ID_Navio, Data_requi, Hora_requi, viagem, Requi_servico, berco_requi, posicao_requi, Obs_requi, Fatu_requi, ID_Agencia)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
    `;
...
app.put('/requisicoes/:id', async (req, res) => {
  ...
  try {
    ...
    const query = `
      UPDATE requisicoes
      SET ID_Navio = ?, Data_requi = ?, Hora_requi = ?, viagem = ?, Requi_servico = ?, berco_requi = ?, posicao_requi = ?, Obs_requi = ?, Fatu_requi = ?
      WHERE ID = ? AND ID_Agencia = ?;
    `;
    ...
    pool.query(
      ...
      async (err, results) => {
        ...
        await sendEmail(
          ...
          updatedRequisicao,
          false,
          'ATUALIZAÇÃO DE REQUISIÇÃO DE SERVIÇOS DE PRATICAGEM'
        ).catch((error) => {
          console.error('Error sending email:', error);
        });
...
app.post('/condicionada', async (req, res) => {
  ...
  try {
    ...
    pool.query(checkQuery, [ID_NavioMain, ID_NavioSub, Data], async (err, results) => {
      ...
      const insertQuery = `
        INSERT INTO Condicionado (ID_NavioMain, ID_NavioSub, Data, Viagem, Servico, Berco, Posicao_Berco, OBS, Fatu, ID_Agencia)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
      `;
...
app.put('/condicionada/:id', async (req, res) => {
  ...
  try {
    ...
    const query = `
      UPDATE Condicionado
      SET ID_NavioMain = ?, ID_NavioSub = ?, Data = ?, Viagem = ?, Servico = ?, Berco = ?, Posicao_Berco = ?, OBS = ?, Fatu = ?
      WHERE ID = ? AND ID_Agencia = ?;
    `;
